    ...
    public void editar(Funcionario obj) {
        if (!nome.getText().equals("")
            ...
            if (!email.getText().equals("") && !email.getText().matches(EMAIL_PATTERN)) {
                ...
                try {
                    //dados endereço
                    obj.getPessoa().getResidencia().setRua(rua.getText());
                    obj.getPessoa().getResidencia().setNumCasa(numero.getText());
                    obj.getPessoa().getResidencia().setBairro(bairros.get(comboBairro.getSelectedIndex()));
                    pai.control.getResidenciaDAO().update(obj.getPessoa().getResidencia());
                    ...
                    obj.getPessoa().setPassaporte(passaporte.getText());
                    obj.getPessoa().setGenero(comboGenero.getSelectedItem().toString());
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        if (opcao == JOptionPane.YES_OPTION) {
            try {
                ...
                if (rows.length != 0) {
                    for (int indice : rows) {
                        int idBairro = list_atual.get(indice).getPessoa().getResidencia().getBairro().getId();
